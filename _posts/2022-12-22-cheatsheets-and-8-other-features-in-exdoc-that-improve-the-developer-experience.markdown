...
ExDoc's Cheatsheets are Markdown files with the  `.cheatmd` extension. One can use it to write cheatsheets for their Elixir projects. Here's an [example of how Ecto is using](https://hexdocs.pm/ecto/crud.html) that.
