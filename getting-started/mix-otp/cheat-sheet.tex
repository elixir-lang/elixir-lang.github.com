...
\path (result.south east)+(3em,0) node (reason) [immediate,anchor=south west,fill=green!25] {
  This applies when \verb|result| matches \verb|{:stop, reason}|, and to \verb|reason| argument in \verb|stop| message.
...
\end{verbatim}
  ...
  \path (result.south east)+(3em,0) node (then-what) [immediate,anchor=south west,fill=green!25] {
    This applies when \verb|result| matches \verb|{_, state, then_what}|
...
\end{verbatim}
  ...
  \path (then-what.north west)+(-1em,0) node [left-title,anchor=north] {\rotatebox{90}{\^{}then\_what =}};
...
\begin{parchment}[\bf\texttt{\{:continue, value\}} $\rightarrow$ \bf\texttt{handle\_continue/2}]
\begin{tikzpicture}
  \node (client) [main-block] {
    \vspace*{-\baselineskip}
\begin{verbatim}
...
\end{verbatim}
  ...
  \path (then-what.north west)+(-1em,0) node [left-title,anchor=north] {\rotatebox{90}{@impl true}};
