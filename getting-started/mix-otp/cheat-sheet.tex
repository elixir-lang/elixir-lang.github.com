...
\end{verbatim}
  ...
  \path (result.north east)+(3em,0) node (then-what) [immediate,anchor=north west,fill=green!30!yellow!18!white] {
    \vspace*{-\baselineskip}
\begin{verbatim}
...
\item[---] Callbacks not listed here are: \verb|code_change/3| and \verb|format_status/2|.
  ...
  \path (then-what.north west)+(-2pt,0) node [left-title,fill=white] {\rotatebox{90}{footnotes}};
