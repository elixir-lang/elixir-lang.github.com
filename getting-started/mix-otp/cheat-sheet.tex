...
\path (result.north west)+(-1em,0) node [left-title,anchor=north,fill=green!25] {\rotatebox{90}{\^{}result = }};

  \draw[color=blue!10!yellow!5,draw=blue!25!yellow,dashed]
        (result.north west)+(0,-38pt) -- ($ (result.north east)+(0,-38pt) $);

\path (result.south east)+(3em,0) node (reason) [immediate,anchor=south west,fill=yellow!30] {
  This applies wherever \verb|result| matches \verb|{:stop, reason}|, and to \verb|reason| argument in \verb|stop| message.
...
\path (reason.north west)+(-1em,0) node [left-title,anchor=north,fill=green!25] {\rotatebox{90}{\^{}reason = }};
...
\end{verbatim}
  ...
  \path (result.south east)+(3em,0) node (then-what) [immediate,anchor=south west,fill=yellow!30] {
    This applies wherever \verb|result| matches \verb|{_, state, then_what}|
...
\end{verbatim}
  ...
  \path (then-what.north west)+(-1em,0) node [left-title,anchor=north,fill=green!25] {\rotatebox{90}{\^{}then\_what =}};
...
\path (result.south east)+(3em,0) node (reply) [immediate,anchor=south west] {user defined};
  \path (reply.north west)+(-1em,0) node [left-title,anchor=north,fill=green!25] {\rotatebox{90}{\^{}reply =}};
...
\end{verbatim}
  ...
  \path (result.south east)+(3em,0) node (then-what) [immediate,anchor=south west,fill=black!5] {
Functions where \verb|@impl true| applies.
\begin{verbatim}
init/1
terminate/2
handle_call/3
...
\end{verbatim}
  ...
  \path (then-what.north west)+(-1em,0) node [left-title,anchor=north,fill=white] {\rotatebox{90}{@impl true}};
