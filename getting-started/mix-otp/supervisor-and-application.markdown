...
But we are not done yet. So far we are supervising the registry but our application is also starting buckets. Since buckets are started dynamically, we can also use a special type of supervisor called `DynamicSupervisor`, which provides some benefits that we will explore next.
