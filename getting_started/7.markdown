...
Both access and update syntaxes above require the given keys to exist. For example, the last line failed because there is no `:c` in the map. This is very useful when you are working with maps on a tight loop and you expect only certain keys to exist.
...
Manipulating maps is done via [the `Map` module](/docs/stable/Map.html) module and that it provides a very similar API to the `Keyword` module. This is because both modules implement the `Dict` behaviour.
...
This interface is defined in the [the `Dict` module](/docs/stable/Dict.html) module which also provides an API that delegates to the underyling implementations:
