...
In the example above, we have assigned the head of the list to the variable `head` and the tail of the list to the variable `tail`. Finally, notice both lists and tuples implement the access protocol, this allows us to access them using indexes:
...
Notice keywords is nothing more than a list of two element tuples, where the first element of the tuple is an atom. It is important to notice that keywords are ordered according to the ordering of the terms and not according to insertion order:

    iex> [foo: 1, bar: 2]
    [{:bar,2},{:foo,1}]

The reason for such ordering is faster access when reading and updating the dictionary. Keywords lists also implement the access protocol, which allows a developer to quickly access an atom key:
...
Since keywords lists are lists, using integers access the tuple in the underlying list:
