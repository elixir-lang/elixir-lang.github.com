...
Notice that `10 / 2` returned a float `5.0` instead of an integer. This is expected. In Elixir, the operator `/` always returns a float. If you want to do integer division or get the division remainder, you can invoke the `div` and `rem` functions:
...
> Note: Functions in Elixir are indentified by name and by number of arguments (a.k.a arity). Therefore, `is_boolean/1` identifies a function named 'is_boolean' that takes 1 argument. `is_boolean/2` identifies a different (nonexistent) function with the same name but different arity.
...
Throughout this tutorial and in Elixir documentation, you will see functions mentioned in the format `is_boolean/1`. The first part is the function name and the number is the arity, i.e. the number of arguments the function expects.
...
The [String module](/docs/stable/String.html) contains a bunch of functions that operate on strings as defined in the Unicode standard:
...
Keep in mind that a variable assigned to inside a function does not affect its surrounding environment:
...
Tuples stores elements contiguously in memory. This means accessing a tuple element per index or getting the tuple size is a fast operation (indexes start from zero):
...
It is also possible to set an element at a particular index in a tuple with `set_elem/3`:
...
Notice that `set_elem/3` returned a new tuple. The original tuple stored in the `tuple` variable was not modified because Elixir data types are immutable. By being immutable, Elixir code is easier to reason about as you never need to worry if a particular code is mutating your data structure in place.

By being immutable, Elixir also helps eliminate common cases where concurrent code have race conditions because two different entities are trying to change a data structure at the same time.
...
Tuples, on the other hand, are stored contiguously in memory. This means getting the tuple size or accessing an element by index is fast. However, updating or adding element to tuples is expensive because it requires copying the whole tuple in memory.

Those performance characteristics dictates the usage of those data structures. For example, one very common use case for tuples is to use them to return extra information from a function, as seen in the `File.read/1` function:
...
If the path given to `File.read/1` exists, it returns a tuple with the atom `:ok` as first element and the file contents as second. Otherwise, it returns `:error` and the error reason.
...
When "counting" the number of elements in a data structure, Elixir also abides to a simple rule: the function should be named `size` if the operation is in constant time (i.e. the value is pre-calculated) or `length` if the operation requires explicit counting.
