...
Clauses also support special conditions to be specified via guards:
...
Keep in mind errors in guards do not leak but simply the guard fail:
...
`case` is useful when you need to match against different values. However, in many circunstances, we want to check different conditions and find the first one that evaluates to true. In such cases, one may use `cond`:
...
If none of the conditions return true, an error is raised. For this reason, it is common to see a last condition equal to `true`, which will always match:
...
One curiosity about `if/2` and `unless/2` is that they are implemented as macros in the language, they are not keywords (as they would be in many languages). You can check the documentation and the source of `if/2` in [the `Kernel` module docs](http://elixir-lang.org/docs/stable/Kernel.html). The `Kernel` module is also where operators like `+/2` and functions like `is_function/2` are defined, all automatically imported and available in your code by default.
...
Which leads to an undefined function error, since `do` is being passed to the `is_number` function, which is then being called with two arguments, and `if` is being called with only one argument. Adding explicit parentheses is enough to resolve the ambiguity:
...
Although it is rare to use the keyword syntax with `case` and `cond`, we will see keyword lists play an important role in the language and are quite common in my functions and macros. We will explore them a bit more in a future chapter, but before, it is time to talk about "Binaries, strings and char lists".