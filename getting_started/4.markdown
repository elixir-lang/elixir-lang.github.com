...
Also, in Elixir parlance, we don't say a variable is assigned. We say a variable is bound or not. A variable can only be bound just on the left side of `=`:
...
The match operator is not only used to match against simple values but they are also useful for destructuring more complex data types. For example, we can pattern match on tuples:
