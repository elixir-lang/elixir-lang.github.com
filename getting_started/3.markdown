...
In order to support software-reuse, Elixir supports two directives. As we are going to see below, they are called directives because they are the only functions in Elixir that have **lexical scope**.
...
An attempt to call a macro that was not loaded will raise an error. Note that, as the `import` directive, `require` is also lexically scoped.
