...
* `@after_compile` - expects a tuple `{ module, function }` containing a module and a function which will be invoked right after the current module is compiled. The module environment and its bytecode are passed as arguments to the callback function;
* `@before_compile` - expects a `{ module, function }` containing a module and a function that will be invoked before the module is compiled with the module environment. The function may be a macro, allowing you to inject functions inside the module exactly before compilation;
* `@behaviour` - (notice the british spelling) used for specifying an OTP or user-defined behaviour;
* `@compile` - provides options for the module compilation;
* `@doc` - provides documentation for the next function or macro to be defined;
* `@file` - changes the filename in stacktraces of the next function or macro to be defined;
* `@moduledoc` - provides documentation for the current module;
* `@on_definition` - expects a `{ module, function }` containing a module and a function which will be invoked after each function/macro is defined. This makes it easy to annotate and customize functions;
* `@on_load` - expects a function name from the currently being defined module that will be invoked whenever the module is loaded. The function must have arity 0 and has to return `:ok`, otherwise the loading of the module is aborted;
...
In other words, we are simply calling the function `flatten` on the atom `:list`. This mechanism is exactly what empowers Elixir aliases. An alias in Elixir is a capitalized identifier (like `List`, `Keyword`, etc) which is converted to an atom representing a module during compilation. For instance, the `List` alias translates by defailt to the atom `Elixir-List`:
...
> Note: in the future, the alias `List` will be converted to the atom `Elixir.List` instead of `Elixir-List`.
