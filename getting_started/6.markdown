...
Internally, `%b` is translated as a function call to `__b__`. For instance, the docs for `%b` are available [in the function `__b__/1` defined in Elixir.Builtin module](http://elixir-lang.org/docs/Elixir.Builtin.html#__b__/1).
...
In the example, we use heredocs to allow the documentation to span over several lines and use markdown to style the documentation.
