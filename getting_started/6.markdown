...
Internally, `%b` is translated as a function call to `__b__`. For instance, the docs for `%b` are available [in the function `__b__/1` defined in `Kernel` module](http://elixir-lang.org/docs/Kernel.html#__b__/1).
...
All functions can be partially applied, except [Elixir's special forms](http://elixir-lang.org/docs/latest/Kernel.SpecialForms.html).
