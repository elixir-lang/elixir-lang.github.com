...
You can see that, instead of containing bytes, a char list contains the codepoints of the characters in between single-quotes (note that chars in the ASCII range are displayed as chars). So while double-quotes represents a string (i.e. a binary), single-quotes represents a char list (i.e. a list).
