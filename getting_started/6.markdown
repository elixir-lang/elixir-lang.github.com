...
Keep in mind that `&args` binds only to the closest function call. For example, the following syntax is invalid:

    iex> foo(&1, 1 + &2)
    ** (SyntaxError) iex:1: partial variable &2 cannot be defined without &1

This is because we have two functions calls in the example above, `foo` and `+`, and `&1` binds to `foo` while `&2` binds to `+`. In such cases, you need to use the usual function syntax:
...
This allows `ExUnit.Case` to configure and inject the proper setup into the `AssertionTest` module.
