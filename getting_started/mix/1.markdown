...
The dependency is represented by an atom, followed by a requirement and some options. By default, Mix uses [hex.pm](https://hex.pm) to fetch dependencies but it can also fetch them from a git repository or directly from the file system.
