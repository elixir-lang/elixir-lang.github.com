...
### 1.1.2 Compiling from source (Unix)
...
On Windows, precompiled packages are the way to go for development. However, keep in mind that deployment on Windows platforms is not advised as both Elixir and the Erlang VM do not recommend deployments of any systems on Windows hosts.
