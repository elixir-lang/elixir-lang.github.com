...
title: 5 Supervisors and Applications
guide: 5
---
...
So far our application requires an event manager, a registry and may potentially use dozens, if not hundreds of buckets. While we may think our implementation so far is quite good, no software is bug free, and failures are definitely going to happen.

When things are failing, our first reaction may be: "let's rescue those errors". But we have learned in the Getting Started guide that in Elixir we don't program defensively nor have the habit of rescuing exceptions, as commonly seen in other languages. Instead, we say "fail fast" or "let it crash". If there is a bug that leads our registry to crash, we have nothing to worry about because we are going to setup a supervisor that will start a new, fresh copy, of the registry.
...
Finally keep in mind that if you want to create a new application that defines a supervisor and the application callback upfront, you can simply pass the `--sup` option to `mix new`, for example:

    $ mix new --sup sample

Give the command above a try and compare the generated application with the one we have so far. What are the differences? Remember to reach to [the Supervisor module documentation](/docs/stable/elixir/Supervisor.html) if you have questions.
