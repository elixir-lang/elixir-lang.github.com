...
Finally, note the `mix run` command also accept a `--config` flag, which allows configuration files to be given on demand. This could be used to start different nodes, each with their specific configuration (for example, different routing tables).
