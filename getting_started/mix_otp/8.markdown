...
We fixed the issue for the registry by using a simple one for one supervisor. We are going to use the same solution here, except that this pattern is so common with tasks, that tasks already come with a simple one for one supervisor that we can just use in our supervision tree!
...
Before we end this section, there are is one detail we have glanced over that we need to revisit. Is our supervisor strategy is still appropriate?

We must always ask this question when we change the supervisor specification. In this case, the answer is yes: if the acceptor crashes, there is no need to crash the existing connections. On the other hand, if the task supervisor crashes, there is no need to crash the acceptor too. This is a contrast to the registry, where we initially had to crash the supervisor every time the registry crashed, until we used ETS to persist state. However tasks have no state and nothing will go stale if one of those process die.

In the next chapter we will start parsing the client requests and send the proper responses, so we can fully implement our server.
