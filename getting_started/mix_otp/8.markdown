...
Start a new server with `mix run --no-halt` and we can now open up many concurrent telnet clients. You will also notice that quitting a client does not bring the acceptor down. This is good news but something is missing. Quitting a client indeed does not bring the acceptor down, but the serve task still crashes. Why isn't anything being logged?

When using the Elixir and OTP behaviours, their logging functionality is disabled by default. We can enable it by starting [Erlang's `sasl` application](http://www.erlang.org/doc/man/sasl_app.html). We can start it in two ways. One is in a case by case basis by passing `--app sasl` when starting Elixir:

    $ elixir --app sasl -S mix run --no-halt

Now you can see a lot of information about the system is logged, including which application, supervisor and server are started. Now if you connect with a client and close it, you get an error report.

In case you find the SASL output useful, you can always start it by default by specifying it in your applications list inside `mix.exs`:

```elixir
def application do
  [applications: [:sasl, :kv],
   mod: {KVServer, []}]
end
```

We will fix the client connection errors in the next chapter. For now, here is the full echo server implementation, in a single module:
