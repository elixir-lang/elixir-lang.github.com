...
defmodule Todo.ManagerTest do
  ...
  test "spawns todo lists", %{manager: manager} do
    ...
    assert Todo.Manager.create(manager, "shopping") == :ok
...
Look that we were able to change considerably the server implementation without changing any of the client API. That's one of the benefits of explicitly segregating the server and the client and providing proper client APIs like `Todo.Manager.start_link/1`.

## 3.4 Monitors or links?
