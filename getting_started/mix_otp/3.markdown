...
defmodule KV.Registry do
  ...
  def handle_call({:lookup, name}, _from, names) do
    {:reply, Dict.fetch(names, name), names}
  end

  def handle_cast({:create, name}, names) do
    if Dict.has_key?(names, name) do
      {:noreply, names}
    else
      {:ok, bucket} = KV.Bucket.start_link()
      {:noreply, Dict.put(names, name, bucket)}
    end
  end
end
```
...
def handle_cast({:create, name}, {names, refs}) do
  if Dict.has_key?(names, name) do
    ...
    refs = Dict.put(refs, ref, name)
    names = Dict.put(names, name, pid)
    {:noreply, {names, refs}}
  end
end

def handle_info({:DOWN, ref, :process, _pid, _reason}, {names, refs}) do
  {name, refs} = Dict.pop(refs, ref)
  names = Dict.delete(names, name)
  {:noreply, {names, refs}}
end
