...
In the previous chapter we have used agents to represent our buckets. In the first chapter, we have specified we would like to name each bucket too:
...
The registry needs to guarantee the dictionary is always up to date. For example, in case one of the bucket processes crashes (sometimes due to a bug), the registry must clean up the dictionary in order to avoid serving stale entries. In Elixir, we then say the registry needs to *monitor* each bucket.

Since the registry process needs to monitor besides keeping the dictionary state, agents are not a good fit for modelling the registry. Instead, we will use a [GenServer](/docs/stable/elixir/GenServer.html) which is the go-to abstraction for building generic servers in both Elixir and OTP.
...
The first function is `start_link/1` which starts a new GenServer passing three arguments: the module where the server callbacks are implemented, in this case the `__MODULE__` itself, the initialization arguments, in this case the atom `:ok`, and a set of options which can, for example, be the name of the server.

The next two functions, `lookup/2` and `create/2` are responsible to send requests to the server. Those requests are often specified as tuples and we have used `{:lookup, name}` and `{:create, name}` respectively.
...
For `cast` requests, we must implement a `handle_cast/2` callback that receives the `request` and the current server state (`names`). The `handle_call/3` callback returns a tuple in the format `{:noreply, new_state}`.
...
To shutdown the registry, we are simply sending a `:shutdown` signal to its process. While this solution is ok for tests, if there is a need to stop a `GenServer` as part of the application logic, it is best to define a `stop/1` function that sends a `call` message causing the server to stop:
...
Our registry is almost complete. The only remaining issue is that the registry may become stale in case a bucket dies. Let's add a test to `KV.RegistryTest` that exposes this bug:

```elixir
test "removes buckets on exit", %{registry: registry} do
  KV.Registry.create(registry, "shopping")
  {:ok, bucket} = KV.Registry.lookup(registry, "shopping")
  kill_with bucket, &Agent.stop/1
  assert KV.Registry.lookup(registry, "shopping") == :error
end
```

The test above will fail on the last assertion as the bucket name remains on the registry even after we stop the bucket process.
...
Observe we were able to considerably change the server implementation without changing any of the client API. That's one of the benefits of explicitly segregating the server and the client.
...
2. `handle_cast/2` must be used for asynchronous requests, when you don't care about a reply. A cast does not even guarantee the server has received the message and, for such reasons, must be used sparingly;
...
Since any message, including the ones sent via `send/2` go to `handle_info/2`, there is a chance unexpected messages will arrive to the server. Therefore, if we don't define the `catch-all` clause, those messages could lead our supervise to crash, because no clause would match.
...
If you watch closely our `handle_cast/2` implementation, you can see the registry is both linking and monitoring the buckets:
