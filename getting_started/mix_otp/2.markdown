...
Our first test is quite straight-forward. We start a new `KV.Bucket` and perform some `get/2` and `put/3` operations on it, finally using `Agent.stop(bucket)` to stop the bucket process.
...
defmodule KV.BucketTest do
  ...
  setup do
    {:ok, bucket} = KV.Bucket.start_link
    on_exit(fn -> Agent.stop(bucket) end)
    {:ok, bucket: bucket}
  end
...
We have first defined a setup callback with the help of the `setup/1` macro. The `setup/1` callback runs before every test, in the same process as the test itself. During setup, we have created a new bucket and also registered an `on_exit/1` callback that stops the bucket process once the test is done (more specifically, once the test process exits).
