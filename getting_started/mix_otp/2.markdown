...
defmodule Todo.ListTest do
  ...
  setup do
    {:ok, todo} = Todo.List.start_link
    on_exit(fn -> Agent.stop(todo) end)
    {:ok, todo: todo}
  end
...
We have first defined a setup callback with the help of the `setup/1` macro. The `setup/1` callback runs before every test, in the same process as the test itself. During setup, we have spawned the the `Todo.List` process and also registered an `on_exit/1` callback that will be invoked once the test is done (more specifically, once the test process exits).

Notice that we need a mechanism to pass the `todo` pid from the callback to the test. We do so by using the test context. When we return `{:ok, todo: todo}` from the callback, ExUnit will merge the second element of the tuple (a dictionary) into the test context. The test context is a map which we can then match during the test definition:
...
This distinction is important in case there are expensive actions to be done and you must consider if it will be better to perform such in the client or in the server. For example:
...
When a long action is performed in the server, all other requests to that particular server will have to wait until the long action is done. This may eventually lead to timeouts.
