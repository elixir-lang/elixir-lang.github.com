...
Sigils can also be used to do compile-time work with the help of macros. For example, regular expressions in Elixir are compiled during compilation time, therefore skipping the compilation step at runtime. If you have interest on the subject, we recommend you to check to learn more about macros and check how those sigils are implemented in the `Kernel` module.
