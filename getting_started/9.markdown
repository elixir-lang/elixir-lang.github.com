...
A function may have many "clauses" or definitions which will be executed when the arguments passed to the function match according to the pattern matching rules seen earlier and any guards on the clause evaluate to true.
...
But what if we instead want to add 5 to all of the values in our list?

```elixir
defmodule Math do
  def double_each([head|tail], accumulator) do
    double_each(tail, accumulator ++ [head * 2])
  end

  def double_each([], new_list) do
    new_list
  end
end
...
Here we have used our recursive capabilities to traverse a list doubling each element and returning a whole new list. The process of taking a list and "mapping" over it is known as a "map" algorithm.
...
Our reduce example can even be written with the `sum/1` function:
